...
- Requires the bundled evidence-first method panel for plain-language reasoning, demand hypotheses, counterevidence, and reversible next steps.
...
The Skill invokes xbrief doctor --json before fetching. It preserves the archive even when analysis cannot proceed, and it stops rather than claiming that comments or external facts were fully verified.
...
Comments and JSONL files retain the full fetched reply set. The compact context is only the representative sample supplied to the analysis task. Opportunity cards use the repository's E0–E5 evidence ladder and must not treat engagement as proof of demand.

## Security and limits

XBrief permits only two fixed read-only backend calls: get_tweet and get_tweet_replies. It rejects unsafe cookie-file permissions and symlinks, redacts authentication-like values from backend errors, and never accepts arbitrary backend tool calls.
